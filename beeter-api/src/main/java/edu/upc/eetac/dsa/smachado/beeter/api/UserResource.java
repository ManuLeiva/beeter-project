...
public class UserResource {
	...
	public User createUser(User user) {
		...
		try {
			...
			conn.setAutoCommit(false);//desactivamos para no tener los zombies
			...
			conn.commit();//hacemos el commit para las 2 bases de datos
		} catch (SQLException e) {
			if (conn != null)
				try {
					conn.rollback();
			...
			try {
				...
				if (stmtInsertUserIntoUserRoles != null)
					stmtGetUsername.close();
				conn.setAutoCommit(true);//activamos el commit otra vez
				conn.close();
			} catch (SQLException e) {
			}
		}
	...
	public User login(User user) {
		...
		String pwdDigest = DigestUtils.md5Hex(user.getPassword());//nos da el dm5 en hexadecimal
		String storedPwd = getUserFromDatabase(user.getUsername(), true)
				.getPassword();

		user.setLoginSuccessful(pwdDigest.equals(storedPwd));//compara que los md5 sean los mismos
		user.setPassword(null);//
		return user;
	}
